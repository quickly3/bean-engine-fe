...
export default class BubbleComponent implements OnInit, OnChanges {
  ...
  d3ArrayBin(): void {
    if (this.data.length > 20) {
      ...
      const thresholdFreedmanDiaconis = d3
        .bin()
      ...
      console.log(
        ...
        thresholdFreedmanDiaconis(valueArr)
      );
  ...
  BubbleChart(
    ...
    const root = d3
      ...
      .padding(padding)(d3.hierarchy({ children: I }).sum((i: any) => V[i]));
    ...
    this.svg = d3.select(eleSelector).html('');

    this.svg = d3.select(eleSelector).append('svg');
