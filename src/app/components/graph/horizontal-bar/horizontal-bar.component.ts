...
export default class HorizontalBarComponent {
  ...
  BarChart = (
    data: any,
    {
      x = (d: any) => d, // given d in data, returns the (quantitative) x-value
      y = (d: any, i: any) => i, // given d in data, returns the (ordinal) y-value
    ...
    svg
      ...
      .call((text) =>
        text
          .filter((i) => xScale(X[i]) - xScale(0) < 30) // short bars
          .attr('dx', +4)
